package com.tongcheng.qichezulin.pulltorefresh;
 ...
 * @author 陈靖
 */
public class PullToRefreshLayout2ToMain extends RelativeLayout {
    ...
    // 释放加载的距离
    private float loadmoreDist = 300;
    ...
    public PullToRefreshLayout2ToMain(Context context) {
        this(context, null, 0);
    }

    public PullToRefreshLayout2ToMain(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PullToRefreshLayout2ToMain(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void refreshFinish(final int refreshResult) {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (pullDownY > 0) {
                    // 刷新结果停留1秒
                    new RemainHandler(PullToRefreshLayout2ToMain.this).sendEmptyMessageDelayed(0, 1000);
                } else {
                    changeState(DONE);
                    hide();
                }
    ...
    public interface OnPullListener {
         ...
         * 刷新操作
         */
        void onRefresh(PullToRefreshLayout2ToMain pullToRefreshLayout);

        /**
         * 加载操作
         */
        void onLoadMore(PullToRefreshLayout2ToMain pullToRefreshLayout);
    }
    ...
    static class RemainHandler extends Handler {
        private WeakReference<PullToRefreshLayout2ToMain> mLayout;

        public RemainHandler(PullToRefreshLayout2ToMain layout) {
            mLayout = new WeakReference<>(layout);
        }

        @Override
        public void handleMessage(Message msg) {
            PullToRefreshLayout2ToMain layout = mLayout.get();
            if (null != layout) {
    ...
    static class UpdateHandler extends Handler {
        ...
        public UpdateHandler(PullToRefreshLayout2ToMain layout) {
            mLayout = new WeakReference<>(layout);
        }
    ...
    private class AutoRefreshAndLoadTask extends
        ...
        protected void onPostExecute(String result) {
            ...
            // 刷新操作
            if (mListener != null)
                mListener.onRefresh(PullToRefreshLayout2ToMain.this);
            if (null != mOnRefreshProcessListener) {
                mOnRefreshProcessListener.onStart(refreshView,
                        OnPullProcessListener.REFRESH);
            }
